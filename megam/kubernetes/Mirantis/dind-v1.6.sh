...
function dind::set-build-volume-args {
  dind::step "METHOD dind::set-build-volume-args STARTS "
  if [ ${#build_volume_args[@]} -gt 0 ]; then
    return 0
  fi
  build_container_name=
  if [ -n "${KUBEADM_DIND_LOCAL:-}" ]; then
    dind::say "if true local KUBEADM_DIND_LOCAL "
    build_volume_args=(-v "$PWD:/go/src/k8s.io/kubernetes")
  else
    dind::say "else not local"
  ...
  dind::step "METHOD dind::set-build-volume-args ENDS "
}
...
function dind::check-binary {
  dind::say "check-binay  $1 is set as command "
  ...
  dind::say "build_volume_args  ${build_volume_args[@]}"
  if [ -n "${KUBEADM_DIND_LOCAL:-${force_local:-}}" ]; then
    dind::say "yes kubeadm dind local"
...
function dind::ensure-downloaded-kubectl {
  ...
  dind::say "local kube version ${LOCAL_KUBECTL_VERSION}"
  ...
  if [[ ! -f "${path}" ]]; then
    dind::say "kubectl download on ${path}"
    mkdir -p "${KUBECTL_DIR}"
  ...
  dind::say "make link ${link_target}   ${KUBECTL_DIR}/kubectl"
...
function dind::ensure-kubectl {
  ...
  if [[ ! ${use_k8s_source} ]]; then
    ...
    echo "if enterd ensure downloaded kubectl"
...
function dind::run {
  ...
  dind::say "args for dind_container ${args[@]}"
  dind::say "opts ${opts[@]}"
  dind::say "volume_name   kubeadm-dind-${container_name}"
...
function dind::set-master-opts {
  dind::say "set-master-opts ${BUILD_KUBEADM} ${BUILD_HYPERKUBE} "
  master_opts=()
  if [[ ${BUILD_KUBEADM} || ${BUILD_HYPERKUBE} ]]; then
    # share binaries pulled from the build container between nodes
    dind::yellow "ensule-volumes"
    ...
    dind::say "${force_make_binaries}"
    dind::yellow "bins - ${bins[@]}"
    dind::yellow "master opts - ${master_opts[@]}"
...
function dind::init {
  ...
  dind::say "kubeadm init  $kubeadm_join_flags"
...
function dind::restore {
  dind::say "restore starts ${dind_ip_base}.2"
  local master_ip="${dind_ip_base}.2"
  dind::down
...
function dind::check-for-snapshot {
  if ! dind::volume-exists "kubeadm-dind-kube-master"; then
    dind::say "kube-master volume not exist"
    return 1
  fi
  for ((n=1; n <= NUM_NODES; n++)); do
    if ! dind::volume-exists "kubeadm-dind-kube-node-${n}"; then
      dind::say "kube-node $n volume not exist"
      return 1
    fi
  done
}
...
case "${1:-}" in
  up)
    ...
    if ! dind::check-for-snapshot; then
       dind::say "up dind and take snap"
      force_make_binaries=y dind::up
  #    dind::snapshot
    else
      dind::say "restore dinds "
  #    dind::restore
    fi
    ;;
  reup)
